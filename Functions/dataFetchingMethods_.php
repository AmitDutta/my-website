<?php
    ...
    {
        $str = "select lat, lon, date_format(recorded_datetime, '%M %d %Y %h:%i %p') as recorded_time
                from current_location where STR_TO_DATE(recorded_datetime,'%Y-%m-%d')= '".$date.
                "' order by id";
        ...
        if($resId)
        {
            ...
            {
                $arr[$i] = array('lat'=>$data["lat"],'lon'=>$data["lon"],'time'=>$data["recorded_time"]);
                $i++;
            }
